...
--by-rect::
When picking the target window inside the sibling subtree in `<direction>`, descend by rect
overlap on the perpendicular axis instead of MRU. Lets directional navigation honor visible
position (e.g. `focus right` from a bottom-left window targets the bottom-right window). Only
compatible with `(left|down|up|right)`.

--directional-focus-history::
Remember, per direction, the window focus came from, and return to it instead of the default
(or `--by-rect`) target when focusing back. E.g. `focus left` onto a window records the source,
so a later `focus right` returns to that exact window. The remembered window is used only if it
still exists in the subtree in `<direction>`; otherwise the normal target is used. Only
compatible with `(left|down|up|right)`.
