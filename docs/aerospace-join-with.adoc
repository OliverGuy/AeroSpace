...
[verse]
// tag::synopsis[]
aerospace join-with [-h|--help] [--window-id <window-id>] [--by-rect] (left|down|up|right)
...
--by-rect::
When picking the target node in `<direction>`, descend by rect overlap on the perpendicular
axis instead of MRU. Lets `join-with` honor visible position rather than tree topology.
