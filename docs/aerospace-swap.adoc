...
overlap on the perpendicular axis instead of MRU. Lets `swap` honor visible position (e.g.
`swap right` from a bottom-left window swaps with the bottom-right window). Only compatible
with `(left|down|up|right)`.
