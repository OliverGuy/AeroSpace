...
By default, a newly detected window becomes a sibling of the window that was focused before it
spawned (placed right after it in the same container).

With `split-new-window-with-focused = true`, the new window is instead "split" with the previously
focused window: both are wrapped in a fresh container perpendicular to their parent, the same
structure the xref:commands.adoc#join-with[join-with] command produces. This is closer to i3's
default behavior, where opening a window splits the focused one.

[source,toml]
----
split-new-window-with-focused = false # default
----

[#fullscreen-covers-monitor]
=== Fullscreen covers monitor

With `fullscreen-covers-monitor = true`, a (non-native) fullscreen window is treated as covering its
whole monitor for the xref:commands.adoc#focus[focus] command: the directional `focus` command can't
reach the windows behind it, and any monitor that holds a fullscreen window counts as a single tile.
Focusing such a window by any other means (e.g. `focus --window-id`, clicking it) still exits
fullscreen, as it does without this option.

[source,toml]
----
fullscreen-covers-monitor = false # default
----
